...
#those rasters with only one site do not have a variation. Complete cases removes these raster regions from the results data frame
...
for (i in 1989:2015) {
  plot(completeresultsfullpoint2$averagebio[resultsfullpoint2$year == i] ~ completeresultsfullpoint2$averageS[resultsfullpoint2$year == i], xlab = "Average Species Richness", ylab = "Average Biomass", main = i)
}
...
for (j in unique(resultsfullpoint2$ID)) {
  plot(completeresultsfullpoint2$varbio[completeresultsfullpoint2$ID == j] ~ completeresultsfullpoint2$year[completeresultsfullpoint2$ID == j], ylim = c(0,2000000), ylab = "Variance in Biomass", xlab = "Year", main = j)
}
...
for (k in unique(resultsfullpoint2$ID)) {
   ...
   plot(completeresultsfullpoint2$varbio[completeresultsfullpoint2$ID == k] ~ completeresultsfullpoint2$averageS[completeresultsfullpoint2$ID == k], ylim = c(0,1000), xlim = c(0,40), ylab = "Var in Biomass", xlab = "Average Species Richness", main = k)
}
...
for (k in unique(resultsfullpoint2$ID)) {
  ...
  plot(completeresultsfullpoint2$averagebio[completeresultsfullpoint2$ID == k] ~ completeresultsfullpoint2$averageS[completeresultsfullpoint2$ID == k], ylim = c(0,1000), xlim = c(0,40), ylab = "Biomass", xlab = "Average Species Richness", main = k)
}
...
for (m in vec) {
  averagevarbiovector <- c(averagevarbiovector, var(completeresultsfullpoint2$averagebio[completeresultsfullpoint2$ID == m]))
  averagespeciesrichvector <- c(averagespeciesrichvector, mean(completeresultsfullpoint2$averageS[completeresultsfullpoint2$ID == m]))
  ID <- c(ID, m)
