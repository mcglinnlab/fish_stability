...
S_av <- with(summary_BEF, tapply(S, list(unique_ID), mean, na.rm= T))
...
b_flounder_av <- with(summary_BEF, tapply(flounder_bio, list(unique_ID), mean))
  ...
  #sd over bootstraps
S_bootsd <- with(summary_BEF, tapply(S, list(unique_ID), sd, na.rm = T))
...
sHill_bootsd <- with(summary_BEF, tapply(s_Hill, list(unique_ID), sd))
sasym_bootsd <- with(summary_BEF, tapply(s_asym, list(unique_ID), sd))
...
BEF <- as.data.frame(cbind(b_av, S_av, Spie_av, sN_av,
                           sasym_av, sHill_av, Nind_av, b_shrimp_av,
                           b_flounder_av, B_bootsd, S_bootsd, Spie_bootsd, 
                           sN_bootsd, sHill_bootsd, sasym_bootsd,
...
sHill_yrsd <- with(BEF, tapply(sHill_av, list(ID), sd))
sasym_yrsd <- with(BEF, tapply(sasym_av, list(ID), sd))
...
BEF_yr <- as.data.frame(cbind(b_yrav, B_yrvar, B_yrstab, S_yrav, Spie_yrav, sN_yrav,
                           ...
                           sHill_yrsd, sasym_yrsd, Nind_yrsd, b_shrimp_yrsd,
...
moddat <- as.data.frame(cbind(log2(BEF_yr$b_yrav), log2(BEF_yr$B_yrstab), 
                              ...
                              log2(BEF_yr$S_yrav),log2(BEF_yr$sHill_yrav), log2(BEF_yr$sasym_yrav),
